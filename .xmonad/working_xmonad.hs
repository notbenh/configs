...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch dmenu
    --, ((modm,               xK_p     ), spawn "exe=`dmenu_path | dmenu -fn '-artwiz-lime-*-*-*-*-*-*-*-*-*-*-*-*' -b ` && eval \"exec $exe\"")
    ...
    , ((modm .|. shiftMask, xK_e     ), scratchpadSpawnAction defaultConfig { terminal = myTerminal })

    -- run or raise prompt
    --, ((modm .|. shiftMask, xK_x     ), runOrRaisePrompt defaultXPConfig)
...
--
--myLayout = avoidStruts $ Mirror tiled ||| Full
...
myManageHook = composeAll
    ...
    , className =? "Opera"          --> doF (W.shift "9")
    , className =? "songbird"       --> doF (W.shift "9")
    , className =? "rhytmbox"       --> doF (W.shift "9")
...
myManageHook = scratchpadManageHook (W.RationalRect 0.25 0.375 0.5 0.35) <+>
    ...
    [ [ className =? c --> doFloat        | c <- floats ],
      [ className =? w --> doShift "5"    | w <- comm]   ,
      [ className =? g --> doShift "9"    | g <- media]  ,
      [ resource  =? "desktop_window"  --> doIgnore
      ...
      , isDialog                       --> doCenterFloat
        ...
        <+> manageDocks -- make some space
            where floats = ["Do"]
                  comm   = ["Thunderbird","Pidgin"]
                  media  = ["Opera", "songbird-bin", "rhythmbox"]
...
myLogHook h = dynamicLogWithPP $ dzenPP 
    {
        ppCurrent           =   dzenColor "white" "#444444" . pad
      , ppVisible           =   dzenColor "#3EB5FF" "black" . pad
      , ppHidden            =   dzenColor "#AAAAAA" "black" . pad
...
--myStatusBar = "dzen2 -ta l -tw 650 "
myStatusBar = "dzen2 -ta l -fn '-artwiz-lime-*-*-*-*-*-*-*-*-*-*-*-*' -tw 650 -h 10 "
...
main = do 
   ...
   xmonad $ withUrgencyHook dzenUrgencyHook $ defaultConfig {
      -- simple stuff
        ...
        modMask            = myModMask,
        --numlockMask        = myNumlockMask, -- depricated
